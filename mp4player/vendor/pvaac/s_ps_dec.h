...
struct PS_DEC
{
    ...
    /*
     *  PATCHED (xprs): the row pointers of the four tables above. Upstream
     *  carved them out of a reused SBR buffer as one Int32 slot per pointer,
     *  which only holds when a pointer is 32 bits wide; on a 64-bit host they
     *  overran the delay samples and PS (HE-AACv2) crashed. 5 is the longest
     *  link (aRevLinkDelaySer = {3, 4, 5}).
     */
    Int32 *aaRealDelayRBufferSerQmfRows[NO_SERIAL_ALLPASS_LINKS][5];
    Int32 *aaImagDelayRBufferSerQmfRows[NO_SERIAL_ALLPASS_LINKS][5];
    Int32 *aaRealDelayRBufferSerSubQmfRows[NO_SERIAL_ALLPASS_LINKS][5];
    Int32 *aaImagDelayRBufferSerSubQmfRows[NO_SERIAL_ALLPASS_LINKS][5];
